...
def save_data(data_iter, root_saved_path, files_name):
    train_data = torch.Tensor(data_iter.out_x)
    train_ops = torch.Tensor(data_iter.out_ops)
    train_label = torch.Tensor(data_iter.out_y)

    val_data = torch.Tensor(data_iter.cross_val_x)
    val_ops = torch.Tensor(data_iter.cross_val_ops)
    val_label = torch.Tensor(data_iter.cross_val_y)

    test_data = torch.Tensor(data_iter.test_x)
    test_ops = torch.Tensor(data_iter.test_ops)
    test_label = torch.Tensor(data_iter.test_y)

    torch.save({'samples':train_data, 'labels':train_label}, os.path.join(root_saved_path,files_name, 'train.pt'))
    torch.save({'samples':test_data, 'labels':test_label}, os.path.join(root_saved_path,files_name, 'test.pt'))
    torch.save({'samples':val_data, 'labels':val_label}, os.path.join(root_saved_path,files_name, 'val.pt'))
